...
```bash
vibe-hardening config show              # what's stored, where
vibe-hardening config set telemetry on  # opt in (persisted)
vibe-hardening config set telemetry off # opt out (persisted)
```
...
- **Opt-in anonymous telemetry** — see [Telemetry section](#telemetry-opt-in)

Post-launch:
- Phase 2 begins D+14 (2026-05-27): rule depth based on telemetry hit rates
- agent / MCP rules · Hall of Hardened scorecard · `--fix` auto-apply (under evaluation)
