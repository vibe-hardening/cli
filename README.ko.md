...
# git diff 변경 파일만 스캔 (CI / pre-commit, 큰 repo에서 10배 빠름)
# ref 없으면 HEAD와 비교; ref 있으면 3-dot diff (PR 모드)
...
# baseline과 비교 — PR 모드, 이 PR에서 새로 생긴 finding만 표시
npx vibe-hardening scan --format json -o baseline.json   # main에서 먼저 실행
npx vibe-hardening scan --compare baseline.json          # PR에서 실행
...
게시된 action을 직접 사용 — `uses: vibe-hardening/cli@v1`:
  ...
  scan:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0  # --changed-only PR 모드에 필수
      - uses: vibe-hardening/cli@v1
        with:
          severity: high
          format: html
          output: vh-report.html
          changed-only: origin/${{ github.base_ref }}  # PR 모드 10× 빠름
...
사용 가능 inputs: `cwd` · `severity` · `format` · `output` · `changed-only` · `verify` · `roast` · `version`. critical 또는 high finding이 있으면 exit 1로 종료됩니다.

Output: `exit-code` (`continue-on-error: true`와 조합하면 action을 실패시키지 않고 배포를 gate 가능).

대부분의 팀은 CI에서 `verify: true`를 **실행하지 않습니다** — CI에서 provider에 live API 호출을 보내는 건 rate limit에 걸리기 쉽고, 로컬에서만 실행해도 충분합니다.
...
`vibe-hardening`이 이제 **로컬 AI agent skill 파일**도 스캔합니다 — 코드뿐만 아니라. Skill은 agent 플랫폼이 런타임에 컨텍스트로 로드하는 markdown + scripts이며, 새로운 공격 표면입니다:

```bash
npx vibe-hardening agent scan
```

**Cursor / Claude Code / OpenClaw / Hermes / Gemini CLI / Goose / OpenCode / Codex / Trae / Factory** skill 설치를 자동 감지.

**65개 규칙 / 5개 팩**: A 하드코딩 시크릿(27개 재사용 + `.env`), B 프롬프트 인젝션(11개), C 위험한 shell(14개), D skill 스키마(5개), G MCP 설정(6개).

**왜 지금** — agent 플랫폼 공급망 공격은 불가피. 이것은 조기 경보입니다.
...
- **74개 code 규칙 + 65개 agent-scan 규칙 + 406개 테스트**、일반적인 repo를 5초 이내에 스캔
- **Agent skill scanner** (0.4.0 신규) —— Cursor / Claude Code / OpenClaw / Hermes / Gemini CLI / Goose / OpenCode / Codex / Trae / Factory
